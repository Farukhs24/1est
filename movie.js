...
async function fetchPopularMoviesAndSeries(page = 1, query = '') {
  ...
  const movieEndpoint = query
    ...
    : `${BASE_URL}/discover/movie?api_key=${API_KEY}&page=${page}&language=en-US&primary_release_date.gte=${startDate}&primary_release_date.lte=${endDate}&sort_by=popularity.desc`;

  const seriesEndpoint = query
    ? `${BASE_URL}/search/tv?api_key=${API_KEY}&query=${query}&page=${page}&language=en-US`
    : `${BASE_URL}/discover/tv?api_key=${API_KEY}&page=${page}&language=en-US&first_air_date.gte=${startDate}&first_air_date.lte=${endDate}&sort_by=popularity.desc`;
