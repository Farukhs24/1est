...
async function fetchNewReleasesMoviesAndSeries(page = 1, query = '') {
  ...
  const seriesEndpoint = query
    ...
    : `${BASE_URL}/tv/on_the_air?api_key=${API_KEY}&page=${page}&language=en-US`;
...
async function loadMoviesAndSeries(query = '', page = 1, clear = false) {
  const items = await fetchNewReleasesMoviesAndSeries(page, query);
